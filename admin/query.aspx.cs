...
public partial class admin_query : System.Web.UI.Page
{
    ...
    {
        mycon.Open();
        cmd = new SqlCommand("select * from query where th is null;", mycon);
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            mycon.Open();
            cmd = new SqlCommand("select distinct(uname) from regDetail where type='TH'",mycon);
            dr = cmd.ExecuteReader();
            table.Clear();
            table.Load(dr);
            mycon.Close();
    ...
    protected void LinkButton1_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < GridView1.Rows.Count; i++)
        {
            ...
            if(chk1.Checked==true)
            {
                ...
                cmd = new SqlCommand("update query set th='"+DropDownList1.Text+"' where sno='"+lid.Text+"'",mycon);
                cmd.ExecuteNonQuery();
            }
